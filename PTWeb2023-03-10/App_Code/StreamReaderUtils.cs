...
public sealed class StreamReaderUtils
{
    ...
    public static ReturnValue StreamReader(string RequestURI, byte[] RequestData, Encoding Encode, bool Post)
    {
        ...
        try
        {
            ...
            if (Post)
            {
                if (RequestURI == "https://api.mch.weixin.qq.com/mmpaymkttransfers/sendgroupredpack")
                {/// 裂变红包使用的证书
                    string cert = @"D:\WorkWeb\cert\apiclient_cert.p12";
                    string password = "1259901501";

                    ServicePointManager.ServerCertificateValidationCallback = new RemoteCertificateValidationCallback(CheckValidationResult);
                    X509Certificate cer = new X509Certificate(cert, password);
                    myHttpWebRequest.ClientCertificates.Add(cer);
                }
