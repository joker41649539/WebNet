...
public partial class Fil_MasterPage : System.Web.UI.MasterPage
{
    ...
    private void LoadFoot()
    {
        ...
        try
        {
            iUserID = Convert.ToInt32(Request.Cookies["WeChat_Question"]["USERID"]);
        }
        catch
        {
        ...
        strTemp += "  <li><a class=\"btn btn-app btn-white btn-xs\" href=\"/Question/\"><i class=\"icon-search\"></i>信息查询</a></li>";
        strTemp += "  <li><a class=\"btn btn-app btn-white btn-xs\" href=\"/Question/AddInfo.aspx\"><i class=\"icon-user\"></i>信息录入</a></li>";
        strTemp += "  <li><a class=\"btn btn-app btn-white btn-xs\" href=\"/Question/AddInfos.aspx\"><i class=\"icon-group\"></i>批量导入</a></li>";
        ...
        if (iWeChatID > 0)
        {
            ...
            if (OP_Mode.SQLRUN(strSQL))
            {
                if (OP_Mode.Dtv.Count > 0)
                {
                    ...
                    Response.Cookies["WeChat_Question"]["LOGIN"] = "true";
    ...
    private void WeChatLoad()
    {
        ...
        else
        {
            ...
            if (strWeixin_OpenID == string.Empty || STRUSERID == string.Empty)
            {
                ...
                string strSQL;
                strSQL = " Select * from Question_Users where WeChatOpenID='" + opentid.ToString() + "'";
                ...
                if (OP_Mode.SQLRUN(strSQL))
                {
                    if (OP_Mode.Dtv.Count > 0)
                    {
                        ...
                        Response.Cookies[Constant.COOKIENAMEUSER][Constant.COOKIENAMEUSER_USERID] = OP_Mode.Dtv[0]["ID"].ToString().Trim();
                        Response.Cookies["WeChat_Question"]["USERID"] = OP_Mode.Dtv[0]["ID"].ToString().Trim();
                        Response.Cookies["WeChat_Question"]["COPENID"] = opentid.ToString();
                        Response.Cookies["WeChat_Question"]["CNAME"] = HttpUtility.UrlEncode(UserName);
                        Response.Cookies["WeChat_Question"]["LTIME"] = OP_Mode.Dtv[0]["LTIME"].ToString().Trim();
                        Response.Cookies["WeChat_Question"]["HEADURL"] = HeadUserUrl;
                    ...
                    //    Response.Cookies["WeChat_Question"].Expires = DateTime.MaxValue;
                        ...
                        /// 更新登录时间
                        OP_Mode.SQLRUN("Update Question_Users set Ltime=getdate(),HEADImage='" + HeadUserUrl + "' where WeChatOpenID='" + opentid.ToString() + "'");
                    ...
                    else
                    {
                        try
                        {
                            ...
                            strSQL = " INSERT INTO Question_Users (Nick,HEADImage,WeChatOpenID) VALUES ('" + UserName + "','" + HeadUserUrl + "','" + opentid + "')";
                            ...
                            if (OP_Mode.SQLRUN(strSQL))
                            {
                                if (OP_Mode.Dtv.Count > 0)
                                {
                                    ...
                                    Response.Cookies["WeChat_Question"]["USERID"] = OP_Mode.Dtv[0]["ID"].ToString().Trim();
                                    Response.Cookies["WeChat_Question"]["COPENID"] = OP_Mode.Dtv[0]["WeChatOpenID"].ToString().Trim();
                                    Response.Cookies["WeChat_Question"]["CNAME"] = HttpUtility.UrlEncode(OP_Mode.Dtv[0]["Nick"].ToString()); //HttpUtility.UrlDecode(Request.Cookies["SK_WZGY"]["CNAME"].ToString().Trim(), Encoding.GetEncoding("UTF-8"))
                                    Response.Cookies["WeChat_Question"]["LTIME"] = OP_Mode.Dtv[0]["LTIME"].ToString().Trim();
                                    Response.Cookies["WeChat_Question"]["HEADURL"] = OP_Mode.Dtv[0]["HEADImage"].ToString().Trim();

                                    Response.Cookies["WeChat_Question"][Constant.COOKIENAMEUSER_CNAME] = OP_Mode.Dtv[0]["WeChatName"].ToString().Trim();
                                    Response.Cookies[Constant.COOKIENAMEUSER][Constant.COOKIENAMEUSER_CTX] = OP_Mode.Dtv[0]["HEADImage"].ToString().Trim();
